...
class TrainerAE():
    ...
    def train_one_step(self, iter, viewer):
        ...
        warped_A = np.expand_dims(warped_A, axis=1)
        # target_A = np.expand_dims(target_A, )
        warped_B = np.expand_dims(warped_B, axis=1)
        # target_B = np.expand_dims(target_B, axis=1)

        loss_A = self.model.autoencoder_A.train_on_batch(warped_A, target_A)
        loss_B = self.model.autoencoder_B.train_on_batch(warped_B, target_B)
        print(
            "[{0}] [#{1:05d}] loss_A: {2:.5f}, loss_B: {3:.5f}".format(time.strftime("%H:%M:%S"), iter, loss_A, loss_B),
            end='\r')
